...
//
// Amounts are stored in rupees (orders.amount). Razorpay speaks paise, so the
// only place that converts is the amount check below.

// Timeline entry shown on the admin order page. Never throws: losing a log
// line must not fail a payment.
export async function logOrderEvent(orderId, message, actor = "system") {
  try {
    const { error } = await getSupabase().from("order_events").insert({ order_id: orderId, message, actor });
    if (error) throw error;
  } catch (err) {
    console.error("[orders] could not log event:", err?.message ?? err);
  }
}

export async function createPendingOrder(order) {
  const { data, error } = await getSupabase().from("orders").insert(order).select("id").single();
  if (error) throw error;
  await logOrderEvent(data.id, "Order placed — awaiting payment");
}
...
export async function markOrderPaid({ razorpayOrderId, razorpayPaymentId, amountPaise, currency, source = "razorpay" }) {
  ...
  if (
    (amountPaise != null && amountPaise !== Math.round(Number(order.amount) * 100)) ||
    (currency != null && currency !== order.currency)
  ) {
    return { order, outcome: "mismatch" };
  }

  if (order.payment_status !== "pending") return { order, outcome: "paid" };

  // Guarded on payment_status='pending' so a concurrent caller can't overwrite us.
  ...
  if (!updated) return { order: { ...order, payment_status: "paid" }, outcome: "paid" };

  // Move fulfilment on from "awaiting payment" — but leave an order the admin
  // already cancelled alone (it now shows as Cancelled + Paid, i.e. needs a refund).
  ...
  await logOrderEvent(
    ...
    order.status === "cancelled"
      ? `Payment received (${razorpayPaymentId}) after the order was cancelled — refund needed`
      : `Payment received (${razorpayPaymentId})`,
    source
  );
  return { order: { ...updated, status: confirmed ? "confirmed" : updated.status }, outcome: "paid" };
}
...
// DB as `payment_status = 'paid' and emails_sent_at is null`.
