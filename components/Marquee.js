export default function Marquee({ items }) {
  ...
  return (
    <div className="group relative overflow-hidden border-y hairline bg-paper-soft py-5">
      <div className="animate-marquee flex w-max gap-x-16 group-hover:[animation-play-state:paused]">
        {track.map((item, i) => (
          <div key={i} className="flex items-center gap-x-16">
            <span className="whitespace-nowrap text-[11px] uppercase tracking-[0.3em] text-ink-dim">
              {item}
            </span>
